...
export const getLevelsFactory = (loader: ILoader, parser: IParser) => {
  const run = async (masterPlaylistURI: string, fetchAttempts: number): Promise<Level[]> => {
    try {
      const masterPlaylistText = await loader.tryFetchText(masterPlaylistURI, fetchAttempts);

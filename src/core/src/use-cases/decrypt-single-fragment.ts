...
export const decryptSingleFragmentFactory = (
  ...
  const run = async (key: Key, data: ArrayBuffer, fetchAttempts: number): Promise<ArrayBuffer> => {
    if (!key.uri || !key.iv) {
      return data;
    }
    const keyArrayBuffer = await loader.tryFetchArrayBuffer(key.uri, fetchAttempts);
